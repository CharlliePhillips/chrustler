...
KEY_HEIGHT = 7; // The Z (NOTE: Dish values may reduce this a bit as they carve themselves out)
...
KEY_ROTATION = [0,0,90]; // An example of how you'd rotate a keycap on its side.  Make sure to zoom in on the bottom to make sure it's *actually* going to print flat! This should be the correct rotation for riskeycap profile.  For GEM use:
...
DISH_TYPE = "flat"; // "inv_pyramid", "cylinder", "sphere" (aka "domed"), anything else: flat top
...
// STEM STUFF
STEM_TYPE = "round_cherry"; // "box_cherry" (default), "round_cherry" (harder to print and not as strong), "alps"
...
STEM_OUTSIDE_TOLERANCE_X = 0; // Shrinks the stem a bit on the X axis (both axis for round_cherry)
STEM_OUTSIDE_TOLERANCE_Y = 0; // Shrinks the stem a bit on th Y axix (unused with round_cherry)
